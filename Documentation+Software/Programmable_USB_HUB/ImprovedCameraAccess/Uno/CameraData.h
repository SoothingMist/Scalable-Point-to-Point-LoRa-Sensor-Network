...
bool InitializeCamera()
{
  ...
  if(pixy2Camera.init() == PIXY_RESULT_OK)
  {
    ...
    #ifdef DEBUG
      Serial.println(F("\nPixy2 initialized."));
      Serial.print(F("Camera Height (Rows): ")); Serial.println(imageHeight);
      Serial.print(F("Camera Width (Columns): ")); Serial.println(imageWidth);
    #endif
    return true;
  }
  else
...
// Forward a message through the serial port.
bool ForwardMessage()
{
  #ifdef DEBUG
    static long count = 0;
    count++;
    Serial.print(F("("));Serial.print(count); Serial.print(F(") Sent message of length "));
    Serial.print(MESSAGE[0]); Serial.print(F(" # Pixels: ")); Serial.print(MESSAGE[5]);
    Serial.print(F(" Mem: ")); Serial.println(freeMemory());
    for(int b = 0; b < MESSAGE[0]; b++) Serial.write(MESSAGE[b]);Serial.println();
  #else
    for(int b = 0; b < MESSAGE[0]; b++)
    {
      while(Serial.availableForWrite() < 1) Wait(100);
      Serial.write(MESSAGE[b]);
    }
  #endif
  
  Serial.flush();
  Wait(2000); // worked well for the Arduino Uno R3 case
  //Wait(7000); // worked well for the Arduino Mega 2560
}
