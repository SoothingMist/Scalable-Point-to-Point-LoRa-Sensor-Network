...
if __name__ == '__main__':
  ...
  while True:
    ...
    if PORT_A.in_waiting > 0:
      ...
      messageLength = int.from_bytes(MESSAGE[0], sys.byteorder)
      print("{:.2f}".format(time.time()), " > PORT_A:  ", end = "")
      if PORT_A == TRANSCEIVER_PORT:
        for i in range(1, messageLength):
          print(MESSAGE[i].decode('utf-8'), end = "")
        print()
      else:
        c = ((int.from_bytes(MESSAGE[1], sys.byteorder) << 8) |
             int.from_bytes(MESSAGE[2], sys.byteorder))
        r = ((int.from_bytes(MESSAGE[3], sys.byteorder) << 8) |
             int.from_bytes(MESSAGE[4], sys.byteorder))
        print("Pixel C R ", c, r, ' | RGB ', int.from_bytes(MESSAGE[5], sys.byteorder),
              int.from_bytes(MESSAGE[6], sys.byteorder),
              int.from_bytes(MESSAGE[7], sys.byteorder))
    ...
    if PORT_B.in_waiting > 0:
      ...
      messageLength = int.from_bytes(MESSAGE[0], sys.byteorder)
      print("{:.2f}".format(time.time()), " > PORT_B:  ", end="")
