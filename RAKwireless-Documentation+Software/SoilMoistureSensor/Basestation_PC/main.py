...
if __name__ == '__main__':
  ...
  notDone = True
  dataStorage = open("data.csv", "wt") # https://www.w3schools.com/python/python_file_handling.asp
  if dataStorage is None:
    print("Unable to open data storage file. Program terminates.")
    exit(1)
  dataStorage.write("Date Time,Source ID,Message ID,Sensor Name, Sensor Reading\n")
  while notDone:
    ...
    if message[LOCATION_MESSAGE_TYPE] == 3: # Check for message type 3, a text message
      ...
      dateTime =  str(timeStruct.tm_year) + "-" + str(timeStruct.tm_mon) + "-" + str(timeStruct.tm_mday) + " "
      dateTime += str(timeStruct.tm_hour) + ":" + str(timeStruct.tm_min) + ":" + str(timeStruct.tm_sec) + ","
      messageID = (message[LOCATION_MESSAGE_ID] << 8) | message[LOCATION_MESSAGE_ID + 1]
      dateTime += str(message[LOCATION_SOURCE_ID]) + "," + str(messageID) + ","
      contents = ""
      ...
      if contents == EXPECTED_FINAL_MESSAGE: notDone = False
        ...
        contents = contents[0 : len(contents) - 2] # https://www.w3schools.com/python/python_strings_slicing.asp
        print(contents)
        dataStorage.write(contents + "\n") # https://www.w3schools.com/python/python_file_write.asp
