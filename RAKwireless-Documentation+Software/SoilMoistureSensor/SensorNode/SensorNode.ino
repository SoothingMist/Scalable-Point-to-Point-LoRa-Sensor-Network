...
#define voltageCalibration (6.9387f * mcu_ain_voltage) - 0.0560f // interpret ADC output for known voltage inputs
#define vwcCalibration -(17.741f * pow(voltage_sensor, 4.0f)) + (77.926f * pow(voltage_sensor, 3.0f)) - (95.824f * pow(voltage_sensor, 2.0f)) + (47.161f * voltage_sensor) - 3.6598 // interpret voltage as VWC
float externalSensorVolts; // voltage read from RAK5811
...
void loop()
{
  ...
  // Read the soil-moisture sensors
  externalSensorVolts = readExternalSensor();
...
void send()
{
  ...
  sprintf(thisText, "Batt_V,%7.2f,Temp_C,%7.2f,Humid_%%,%7.2f,Ext_V,%7.2f,|",
          volts, degreesC, percentRH, externalSensorVolts);
...
float readExternalSensor(void)
{
  ...
  float avgVolts; // variable calibrated to give volumetric water content for soil-moisture sensors
	...
	voltage_sensor = voltageCalibration;

  avgVolts = voltage_sensor; //vwcCalibration;
  ...
  avgVolts += voltage_sensor; //vwcCalibration; // assuming two of the same sensors independently measuring the same thing
  avgVolts = avgVolts / 2.0f;
