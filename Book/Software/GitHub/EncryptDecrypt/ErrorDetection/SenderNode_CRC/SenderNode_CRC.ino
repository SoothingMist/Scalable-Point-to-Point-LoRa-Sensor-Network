...
// Basic LoRa Sender Node. Sends messages with CRC code.
...
void setup()
{
  ...
  // Ready
  Serial.println("==============================================");
  Serial.println("Arduino MKR 1310 basic LoRa sender test of CRC");
  Serial.println("==============================================");
}
...
void loop()
{
  ...
  uint16_t crc_code = crcr16dnp((uint8_t*)message, messageLength, 0);
  uint8_t crc_highbyte = crc_code >> 8;
  uint8_t crc_lowbyte = (crc_code << 8) >> 8;

  // Send packet
  LoRa.beginPacket();
  LoRa.write(messageLength); // message length
  LoRa.write(crc_highbyte); // CRC code
  LoRa.write(crc_lowbyte);
  LoRa.write((uint8_t*)message, messageLength); // the message itself
  LoRa.endPacket();
